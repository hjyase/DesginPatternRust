...
impl Publisher {
    pub fn register_listener(&self, listener: Arc<dyn Listener>) {
        if let Ok(mut ls) = self.listeners.try_lock() {
            ls.push(listener);
            println!(
                "register_listener successed, current length is {}",
                ls.len()
            );
        } else {
            println!(
                "register_listener failure, current length is {}",
                self.listeners.try_lock().unwrap().len()
            );
        };
    ...
    pub fn unregister_listener(&self, listener: Arc<dyn Listener>) {
        if let Ok(mut ls) = self.listeners.try_lock() {
            ...
            println!(
                "unregister_listener successed, current length is {}",
                ls.len()
            );
        } else {
            println!(
                "unregister_listener failure, current length is {}",
                self.listeners.try_lock().unwrap().len()
            );
        }
    }
